...
class OracleUseCase {
     ...
     */
    async processClosings(currentDate = new Date()) {
    ...
    async processLiquidations(currentDate = new Date()) {
        ...
        const resolvedPending = await this.marketRepository.findAll({ status: 'RESOLVED' });
        for (const m of resolvedPending) {
            if (!m.oracle_asset || !m.final_price)
                continue;
            // Try settle — idempotent (MarketAlreadySettled is caught and ignored in settleMarketContract)
            try {
                await this.stellarService.settleMarketContract(m.id, 0, m.oracle_asset, m.contract_address ?? null);
            }
        ...
        for (const market of markets) {
            try {
                ...
                if (!marketDetails)
                    continue;
                const winningOutcomeIndex = marketDetails.results.findIndex((r) => r.name.toLowerCase() === winningResultName.toLowerCase());
    ...
    async _resolveOracleMarket(market) {
        ...
        const operator = market.condition_operator ?? 'GREATER_THAN';
        let conditionMet = false;
        if (referencePrice) {
            switch (operator) {
                case 'GREATER_THAN':
                    conditionMet = currentPrice > referencePrice;
                    break;
                case 'LESS_THAN':
                    conditionMet = currentPrice < referencePrice;
                    break;
        ...
        console.log(`[OracleUseCase] Oracle market ${market.id} | asset=${market.oracle_asset} | ` +
        ...
        try {
            ...
            console.error(`[OracleUseCase] On-chain settle_market failed for ${market.id} (DB will still resolve):`, onChainError);
        }
